  ...
  return (
    ...
    >
      <div className={`flex h-9 w-9 items-center justify-center rounded-xl text-lg transition-transform group-hover:scale-110 ${
        isLow ? 'bg-red-500/10 text-red-500' : 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-300'
      }`}>
        {isLow ? '⚠️' : '💎'}
      </div>
      <div className="flex flex-col">
        ...
        <div className="flex items-baseline gap-1">
          ...
          <span className="text-[10px] text-gray-500 dark:text-slate-500 font-bold">/ {safeGrant}</span>
        </div>
      </div>
      {isLow && (
