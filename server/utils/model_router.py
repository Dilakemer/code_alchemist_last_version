...
class ModelRouter:
    ...
    def route(self, language: str, intent: str, user_prefs: dict = None) -> tuple[str, str]:
        ...
        # 1. Language-Based Routing (Highest Priority)
        ...
        # Bash/Shell/DevOps: Gemini 1.5 Flash is fast and reliable for shell commands
        elif language == 'bash':
        ...
        # 3. User Preference Override (Only if no specific language/intent match was specific enough)
        if preferred_model != 'auto' and intent in ['code', 'general']:
            ...
            return chosen, f"⚙️ Auto-routing active. Detected language: **{language}** | Using preferred: **{chosen}**."

        # 4. Default / General Fallback → Gemini 1.5 Flash
